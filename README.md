...
This Partner Solution deploys Salesforce Health Cloud: Virtual Care on the AWS Cloud. 
...
This Partner Solution integrates various AWS services with a Salesforce Health Cloud. To receive support from AWS, you must enroll in either AWS Business Support or a higher support plan. For details and costs, refer to [Compare AWS Support Plans](https://aws.amazon.com/premiumsupport/plans/).

There is no cost to use this Partner Solution, but you will be billed for any AWS services or resources that this solution deploys. 

## Architecture

Deploying this Partner Solution with default parameters builds the following Virtual Care environment in the AWS Cloud.
...
As shown in [Architecture for Virtual Care on AWS](#architecture1), this Partner Solution sets up the following:
...
This Partner Solution provides the following deployment option for provisioning Virtual Care resources into your new AWS account. Configure Virtual Care settings as needed.
...
1. Do the following steps to download the latest template file from GitHub:
   1. Navigate to the [Github source code](https://github.com/aws-ia/cfn-ps-salesforce-health-cloud-virtual-care/) for this Partner Solution to open the GitHub repository. 
...
1. Sign in to your AWS account, and launch this Partner Solution, as described under **Deployment options** above. The AWS CloudFormation console opens with a prepopulated template. 
...
This Partner Solution was created by Salesforce, Inc. in collaboration with Amazon Web Services (AWS). Partner Solutions are automated reference deployments that help people deploy popular technologies on AWS according to AWS best practices.
